...
Connection.prototype.copy = function (src, dest, options, cb) {
  ...
  // Complete dest.
  var completeDest = remote.format(this.remote) + ':' + dest;
  ...
  this.options.logger.log('Remote copy "%s" to "%s"', src, completeDest);
  ...
  if (this.options.stdout)
    child.stdout.pipe(wrapper).pipe(this.options.stdout);

  if (this.options.stderr)
    child.stderr.pipe(errorWrapper).pipe(this.options.stderr);
